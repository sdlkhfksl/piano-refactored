  ...
  <div class="score-container component-auto-list">
    <div class="list-view" v-show="currentView=='LIST'">
      <p class="component-title">
        <img src="/images/music_cd.png" alt="" class="music-img">
      ...
      <p class="component-title">
        <img src="/images/lab.png" alt="" class="music-img">
...
import ScoreXml from '@/config/scorexml' // This will be an empty array for now
...
const clickScoreItem = (item) => {
  ...
  if (playingItem.value.name) {
    pauseAutoPlay(playingItem.value)
  }
  item.playing = true
  ...
  setTimeout(() => {
    window.dispatchEvent(new CustomEvent(OBEvent.AUTO_PLAY_NUM_SCORE, {
      detail: { scorename: item.name }
    }))
  }, 200)
}

const clickXMLScoreItem = async (item) => {
  ...
  try {
    ...
    if (score && typeof score == 'object') {
      // currentView.value = 'PLAYER'
      setTimeout(() => {
        window.dispatchEvent(new CustomEvent(OBEvent.AUTO_PLAY_XML_SCORE, {
          detail: { musicScore: score }
        }))
      }, 200)
    }
  } catch (e) {
...
const pauseAutoPlay = (item) => {
  item = item || playingItem.value
  item.playing = false
  currentView.value = 'LIST'
  window.dispatchEvent(new CustomEvent(OBEvent.PAUSE_AUTO_PLAY, {
    detail: { scoreItem: item }
  }))
  playingItem.value = {
    name: ''
  }
